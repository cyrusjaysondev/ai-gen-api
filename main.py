...
class T2VRequest(BaseModel):
    prompt: str
    negative_prompt: str = "worst quality, inconsistent motion, blurry, jittery, distorted, grainy, noisy, low resolution, pixelated, old film, vhs, vintage, washed out"
    width: int = 544
    height: int = 960
    seconds: int = 5
    seed: int = -1
...
def get_workflow(prompt, negative_prompt, width, height, length, seed, image_filename=None, cfg=1.5, steps=8, audio=True):
    wf = {
        ...
        "4": {"class_type": "LoraLoaderModelOnly", "inputs": {"lora_name": "ltx-2.3-22b-distilled-lora-384.safetensors", "strength_model": 1.0, "model": ["1", 0]}},
...
async def text_to_video(req: T2VRequest, background_tasks: BackgroundTasks):
    ...
    workflow = get_workflow(prompt, req.negative_prompt, req.width, req.height, length, seed, cfg=req.cfg, steps=req.steps, audio=req.audio)
